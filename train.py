...
def train():
    ...
    dataset = BaseDataset(
        "data/train/behaviors_parsed.tsv", "data/train/news_parsed.tsv"
    )
    ...
    if checkpoint_path is not None:
        ...
        # Updated to properly map checkpoint to current device
    ...
    for i in tqdm(
        range(1, config.num_epochs * len(dataset) // config.batch_size + 1),
        desc="Training",
    ):
        try:
        ...
        if model_name == "LSTUR":
            ...
            y_pred_averaged = torch.stack(
                [F.softmax(y_pred, dim=1) for y_pred in y_preds], dim=-1
            ).mean(dim=-1)
        ...
        if model_name == "HiFiArk":
            if i % 10 == 0:
                ...
                writer.add_scalar(
                    "Train/RegularizerLoss", regularizer_loss.item(), step
                )
                writer.add_scalar(
        ...
        if i % config.num_batches_validate == 0:
            ...
            val_auc, val_mrr, val_ndcg5, val_ndcg10 = evaluate(
                model if model_name != "Exp1" else models[0],
                "./data/val",
                config.num_workers,
                200000,
            )
                    ...
                    torch.save(
                        {
                            ...
                            "optimizer_state_dict": (
                                optimizer if model_name != "Exp1" else optimizers[0]
                            ).state_dict(),
                            "step": step,
...
def parse_args():
    ...
    parser.add_argument(
        "--model_name", type=str, default=model_name, help="Name of the model to train"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Device to use (cuda/cpu, overrides auto-detection)",
    )
