...
class OMAP(torch.nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.W = nn.Parameter(
            torch.empty(config.num_filters, config.num_pooling_heads).uniform_(
                -0.1, 0.1
            )
        )
    ...
    def forward(self, self_attended_clicked_news_vector):
        ...
        weights = F.softmax(
            torch.bmm(
                ...
                self.W.expand(real_batch_size, -1, -1),
            ).transpose(1, 2),
            dim=2,
        )
        # batch_size, num_pooling_heads, num_filters
        user_archive_vector = torch.bmm(weights, self_attended_clicked_news_vector)
        ...
        if self.training:
            ...
            # num_pooling_heads, num_pooling_heads
            right = (
                torch.ones(self.config.num_pooling_heads, self.config.num_pooling_heads)
                - torch.eye(self.config.num_pooling_heads)
            ).to(device)
