...
class MacOptimizedConfig(BaseConfig):
    """
    Mac-optimized configuration for MPS to avoid memory issues
    """
    batch_size = 1
    num_workers = 1
    # Reduce memory usage for MPS
    query_vector_dim = 128
    num_clicked_news_a_user = 30  # Reduced from 60
...
class NAML_LSTUR_DKNConfig(BaseConfig):
    ...
    # Use original settings to maintain compatibility with existing checkpoints
    num_filters = 300  # Corrected to match the actual checkpoint
    window_size = 4    # Original setting
    query_vector_dim = 200  # Original setting - must match checkpoint
    masking_probability = 0.5
    # Use DistilBERT to match the checkpoint architecture
    use_distilbert = True  # Corrected - the checkpoint was trained with BERT/DistilBERT
    
    # For MPS, reduce batch size and workers to manage memory
    if torch.backends.mps.is_available():
        batch_size = 1  # Much smaller batch size for MPS
        num_workers = 1  # Reduce workers for MPS
        num_clicked_news_a_user = 30  # Reduce history length for MPS


class NAML_LSTUR_DKN_MacConfig(MacOptimizedConfig):
    """
    Mac-optimized configuration for training new models from scratch on MPS
    Use this for NEW training, not for loading existing checkpoints
    """
    dataset_attributes = {
        "news": ["category", "subcategory", "title", "abstract"],
        "record": ["user", "clicked_news_length"],
    }
    # Reduced dimensions for MPS memory constraints
    num_filters = 256  # Reduced for MPS
    window_size = 3    # Reduced for MPS
    query_vector_dim = 128  # Reduced for MPS
    masking_probability = 0.3  # Reduced for MPS
